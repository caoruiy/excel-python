...
class Excel(object):
    ...
    def write(self, row=0, col=0, content=None, sheet=None):
        ...
        self._clear_error()
        self._sheet_name = sheet or self._sheet_name
        self._write(row, col, content, self._sheet_name)
        return self

    def _write(self, row, col, content, sheet):
        """
        ...
        if isinstance(row, list):
            ...
            for key, val in enumerate(row):
                self._data[str(sheet) + ":" + str(self._last_row) + ":" + str(key)] = [sheet, self._last_row, key, val]
        else:
            self._data[str(sheet) + ":" + str(row) + ":" + str(col)] = [sheet, row, col, content]

    def save(self):
        for item in self._data:
            item = self._data[item]
            try:
                sheet, row, col, content = item
